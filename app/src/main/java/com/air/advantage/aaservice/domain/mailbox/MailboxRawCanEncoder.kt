...
object MailboxRawCanEncoder {
    ...
    private fun zoneConfigData(cfg: JSONObject): ByteArray {
        ...
        fun constantAt(index: Int) = constantIds?.optInt(index, 0)?.coerceIn(0, 255) ?: 0
        return byteArrayOf(
    ...
    private fun unitActivationData(dto: JSONObject): ByteArray {
        ...
        val activation =
            when (dto.optString("activation_status")) {
                "no_code" -> 0x00
                "code_enabled" -> 0x01
                "expired" -> 0x02
                else -> 0x00
    ...
    private fun zoneStateData(
        ...
        val sensor =
            when (zoneDto.optString("sensor_type")) {
                "no_sensor" -> 0x00
                "rf" -> 0x01
                "rf2can_booster" -> 0x03
                "rf_x" -> 0x04
        ...
        val fresh =
            when (freshRaw) {
                is Boolean -> if (freshRaw) 0x02 else 0x01
                    ...
                    when (status.optString("fresh_air", "none").lowercase()) {
                        "on" -> 0x02
                        "off" -> 0x01
                        else -> 0x00
                    }
    ...
    private fun firmwareData(dto: JSONObject): ByteArray =
        byteArrayOf(
    ...
    private fun infoByteData(dto: JSONObject): ByteArray =
        byteArrayOf(
    ...
    private fun rfDevicePairingData(dto: JSONObject): ByteArray =
        byteArrayOf(
    ...
    private fun rfDeviceCalibrationData(dto: JSONObject): ByteArray =
        byteArrayOf(
    ...
    private fun tempCToX2(tempC: Double): Int = (tempC * 2.0).roundToInt().coerceIn(0, 255)
}
