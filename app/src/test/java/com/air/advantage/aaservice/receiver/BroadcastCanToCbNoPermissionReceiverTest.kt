...
class BroadcastCanToCbNoPermissionReceiverTest {
    ...
    @Before
    fun setUp() {
        receiver = BroadcastCanToCbNoPermissionReceiver()
    ...
    @Test
    fun `onReceive decrypts and enqueues CAN IDs`() {
        mockStatic(CryptoHelper::class.java).use { crypto ->
            val encrypted = byteArrayOf(1, 2, 3)
            val decrypted = "10 20 30".toByteArray()
    ...
    fun `onReceive returns early when extra is null`() {
        mockStatic(CryptoHelper::class.java).use {
            val intent = mock(Intent::class.java)
            whenever(intent.getByteArrayExtra("com.air.advantage.BROADCAST_CAN_TO_CB_NO_PERMISSION")).thenReturn(null)
    ...
    fun `onReceive with null service does nothing`() {
        ...
        mockStatic(CryptoHelper::class.java).use { crypto ->
            val encrypted = byteArrayOf(1, 2, 3)
            val decrypted = "10 20".toByteArray()
            crypto.`when`<ByteArray?> { CryptoHelper.decrypt(any()) }.thenReturn(decrypted)
